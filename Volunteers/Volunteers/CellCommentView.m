...
@interface CellCommentView () <MLLinkLabelDelegate>
...
@implementation CellCommentView
...
- (void)setCommentItemsArray:(NSArray *)commentItemsArray
{
    ...
    for (int i = 0; i < commentItemsArray.count; i++) {
        CellCommentItemModel *model = commentItemsArray[i];
...
- (void)commentLabelTapped:(UITapGestureRecognizer *)tap
{
    if (self.didClickCommentLabelBlock) {
        ...
        CellCommentItemModel *model = self.commentItemsArray[tap.view.tag];
